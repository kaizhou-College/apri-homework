...
--提示：首先根据用户输入的员工编号（empno）获得其对应的部门编号（deptno），然后根据部门编号（deptno）判断，更新薪水sal。
select * from emp
declare 
  dept emp%rowtype;
  num number:='&请输入员工编号';
begin
  select * into dept from emp where empno=num;
  if dept.deptno=10 then
     if dept.sal is null then 
      update emp set sal=nvl(sal,0)+2000 where deptno=10;
     else 
     update emp set sal=sal+2000 where deptno=10; 
       ...
       if dept.sal is null then 
                   update emp set sal=nvl(sal,0)+1700 where deptno=20;
       else
                   update emp set sal=sal+1700 where deptno=20;
       end if;
   else
       if dept.sal is null then 
          update emp set sal=nvl(sal,0)+1500 where deptno=30;
       else
          update emp set sal=sal+1500 where deptno=30; 
        end if;
   end if;
   commit;
