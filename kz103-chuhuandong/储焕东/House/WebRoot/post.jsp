<%@page contentType="text/html; charset=utf-8" import="java.util.*,com.kz.entity.TBL_USER"%>
...
-->
	...
	<tr>
		<td width=''><font color='red'>当前用户：
			...
			out.print(list.get(0).getUname()+""+list.get(0).getUuid());
			%></font>
		</td>
	</tr>
	<tr>
	...
	<form action="manager_postcon" method="post" name="mf" onsubmit="return validateTitle()">
		<input type="hidden" name="affirm" value="n"/>
		<input name="fwxx.uuid" value="<%=list.get(0).getUuid() %>" style="display: none"/>
		<input name="fwxx.fwid" value="${fwxx.fwid}" style="display: none" />
		<table>
			<tr>
			...
			<tr>
			  <td><strong>标题：</strong></td>
				<td><input type="text" name="fwxx.title" value="${fwxx.title}"></td>
			</tr>
			<tr>
			  <td><strong>区县：</strong></td>
				<td><select name="qxid" onChange="selectjd()">
					<option value='0'>不限--</option>
<option value='1' <c:if test="${qxid==1}">selected="selected"</c:if> >海淀</option>
<option value='2' <c:if test="${qxid==2}">selected="selected"</c:if> >朝阳</option>
<option value='3' <c:if test="${qxid==3}">selected="selected"</c:if> >崇文</option>
<option value='4' <c:if test="${qxid==4}">selected="selected"</c:if> >宣武</option>
<option value='5' <c:if test="${qxid==5}">selected="selected"</c:if> >东城</option>
<option value='6' <c:if test="${qxid==6}">selected="selected"</c:if> >西城</option>
<option value='7' <c:if test="${qxid==7}">selected="selected"</c:if> >通州</option>
<option value='8' <c:if test="${qxid==8}">selected="selected"</c:if> >丰台</option>
<option value='9' <c:if test="${qxid==9}">selected="selected"</c:if> >昌平</option>
<option value='10' <c:if test="${qxid==10}">selected="selected"</c:if> >大兴</option>
<option value='11' <c:if test="${qxid==11}">selected="selected"</c:if> >门头沟</option>
<option value='12' <c:if test="${qxid==12}">selected="selected"</c:if> >石景山</option>
<option value='13' <c:if test="${qxid==13}">selected="selected"</c:if> >房山</option>
<option value='14' <c:if test="${qxid==14}">selected="selected"</c:if> >顺义</option>
<option value='15' <c:if test="${qxid==15}">selected="selected"</c:if> >密云</option>
<option value='16' <c:if test="${qxid==16}">selected="selected"</c:if> >怀柔</option>
<option value='17' <c:if test="${qxid==17}">selected="selected"</c:if> >延庆</option>
<option value='18' <c:if test="${qxid==18}">selected="selected"</c:if> >平谷</option>
				</select></td>
			</tr>
			<tr>
			...
			<tr>
			  <td><strong>租金：</strong></td>
				<td><input type="text" name="fwxx.zj" value="${fwxx.zj}"/>元/月</td>
			</tr>
			<tr>
			  <td><strong>户型：</strong></td>
				<td><select name="fwxx.shi">
					<option value="1" <c:if test="${fwxx.shi==1}">selected="selected"</c:if> >1</option>
					<option value="2" <c:if test="${fwxx.shi==2}">selected="selected"</c:if> >2</option>
					<option value="3" <c:if test="${fwxx.shi==3}">selected="selected"</c:if> >3</option>
					<option value="4" <c:if test="${fwxx.shi==4}">selected="selected"</c:if> >4</option>
					<option value="5" <c:if test="${fwxx.shi==5}">selected="selected"</c:if> >5</option>
					<option value="6" <c:if test="${fwxx.shi==6}">selected="selected"</c:if> >6</option>
					<option value="7" <c:if test="${fwxx.shi==7}">selected="selected"</c:if> >7</option>
					<option value="8" <c:if test="${fwxx.shi==8}">selected="selected"</c:if> >8</option>
					<option value="9" <c:if test="${fwxx.shi==9}">selected="selected"</c:if> >9</option>
				</select>室<select name="fwxx.ting">
					<option value="1" <c:if test="${fwxx.ting==1}">selected="selected"</c:if> >1</option>
					<option value="2" <c:if test="${fwxx.ting==2}">selected="selected"</c:if> >2</option>
					<option value="3" <c:if test="${fwxx.ting==3}">selected="selected"</c:if> >3</option>
					<option value="4" <c:if test="${fwxx.ting==4}">selected="selected"</c:if> >4</option>
					<option value="5" <c:if test="${fwxx.ting==5}">selected="selected"</c:if> >5</option>
				</select>厅</td>
			</tr>
			<tr>
			  <td><strong>房屋类型：</strong></td>
				<td><select name="fwxx.lxid">
					<option value='0'>不限--</option>
<option value='1' <c:if test="${fwxx.lxid==1}">selected="selected"</c:if> >地下室</option>
<option value='2' <c:if test="${fwxx.lxid==2}">selected="selected"</c:if> >平房</option>
<option value='3' <c:if test="${fwxx.lxid==3}">selected="selected"</c:if> >普通住宅</option>
<option value='4' <c:if test="${fwxx.lxid==4}">selected="selected"</c:if> >公寓</option>
<option value='5' <c:if test="${fwxx.lxid==5}">selected="selected"</c:if> >别墅</option>

				</select></td>
			</tr>
			<tr>
			  <td><strong>联系电话：</strong></td>
				<td><input type="text" name="fwxx.telephone" value="${fwxx.telephone}"></td>
			</tr>
			<tr>
			  <td><strong>联系人：</strong></td>
				<td><input type="text" name="fwxx.lxr" value="${fwxx.lxr}"></td>
			</tr>
			<tr>
			  <td><strong>房屋信息：</strong></td>
				<td><textarea name="fwxx.fwxx" rows="10" cols="40" value="${fwxx.fwxx}"></textarea></td>
			</tr>
			<tr>
				<td colspan="2">
