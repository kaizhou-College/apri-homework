...
border-width:0 0 1px 0;
	...
	<form action="fw_query.action" method="post" name="mf">
	...
	if (o){
		if (o.style.display == "none"){ o.style.display = "block"; } else {o.style.display = "none";}
				...
				<div style="text-align:left;width:88%;">
					<input type="text" name="fd.title" value="${fd.title}">
...
<option value='18'>平谷</option>
			...
			<tr>
				<td>租金：</td>
				<td><input type="text" name="fd.startzj" size="5" class="unnamed1" value="${fd.startzj}" > 至 <input type="text" name="fd.endzj" size="5" class="unnamed1" value="${fd.endzj}"> 元/月</td>
			</tr>
			<tr>
				<td>户型：</td>
				<td><select name="fd.shi" style="width:60">
					<option >不限--</option>
				...
				</select>室 <select name="fd.ting" style="width:60">
					<option >不限--</option>
					<option value="1">1</option>
					<option value="2">2</option>
					<option value="3">3</option>
			...
			<tr>
				<td>房屋类型：</td>
				<td>
				<s:radio list="#{1:'精装',2:'简装',3:'毛胚房',4:'无家具'}"  name="fd.lxid" value="%{fd.lxid}"></s:radio>
				</td>
			</tr>
			<tr>
				<td>发布日期：</td>
				<td><select name="fd.time" style="width:80">
					<option>不限--</option>
...
<table width="88%" cellspacing="0">
				...
				<c:forEach items="${list}" var="list">
					<tr>
						<td><a href="fw_queryId.action?fw.fwid=${list.fwid}">${list.title}</a></td>
				...
				<tr>
					<td colspan='3'>
						<c:if test="${index!=1}">
								<!-- &fd.title=${fd.title}&fd.qxid=${fd.qxid}&fd.jqid=${fd.jqid}&fd.endzj=${fd.endzj}&fd.lxid=${fd.lxid}&fd.startzj=${fd.startzj}&fd.time=${fd.time}&fd.shi=${fd.shi}&fd.ting=${fd.ting} -->
								<a href="fw_query.action?index=1">首页</a>
								<a href="fw_query.action?index=${index-1}">上一页</a>
							</c:if>
							<c:if test="${index==1}">
								<a href="javascript:return false" title="" style="color: gray;">首页</a>
								<a href="javascript:return false" style="color: gray;">上一页</a>
							</c:if>
							<c:if test="${index!=max}">
								<a href="fw_query.action?index=${index+1}" >下一页</a>
								<a href="fw_query.action?index=${max}">尾页</a>
							</c:if>
							<c:if test="${index==max}">
								<a href="javascript:return false" title="" style="color: gray;">下一页</a>
								<a href="javascript:return false" style="color: gray;">尾页</a>
							</c:if>
					</td>
				</tr>
			</table>
  ...
  <tr>
    <td colspan="5"><img src="${basePath}/images/bottom.jpg" width="780" height="82"></td>
			...
			<input type="hidden" name="fd.qxid" value="${fd.qxid}"/>
			<input type="hidden" name="fd.jdid" value="${fd.jdid}"/>
			<input type="hidden" name="fd.lxid" value="${fd.lxid}"/>
			<input type="hidden" name="fw.shi" value="${fd.shi}"/>
			<input type="hidden" name="fd.shi" value="${fd.ting}"/>
			<input type="hidden" name="fd.startzj" value="${fd.startzj}"/>
			<input type="hidden" name="fd.title" value="${fd.title}"/>
			<input type="hidden" name="fd.endzj" value="${fd.endzj}"/>
			<input type="hidden" name="fw.time" value="${fw.time}"/>
</table>
