...
<HTML>
	<HEAD>
		...
		<link href="${basePath}/style/mycss.css" rel="stylesheet"
			type="text/css" />
		<link href="${basePath}/style/texts.css" rel="stylesheet"
			type="text/css" />
		<link href="${basePath}/style/btn.css" rel="stylesheet"
			type="text/css" />
	...
	function selectjd(id){
		...
		for(var i=0;i<col;i++) {
			if(v[row][i]!=0){
				...
				if(document.mf.jdid.options[i+1]==id){
					document.mf.jdid.options[i+1].selected=true;
				}
			}
		}
	}
...
-->
					...
					<table align="center">
						...
						<tr>
							<td width='150'>
								<a href="my.htm">管理我的租房信息</a>
							</td>
						</tr>
						<tr>
							<td width='100'>
								<a href="post.htm">发布租房信息</a>
							</td>
						</tr>
						<tr>
							<td width='100'>
				...
				<td width="495">
					<c:if test="${fw==null}">
							<form action="fw_add.action" method="post" name="mf"
								onsubmit="return validateTitle()">
					</c:if>
					<c:if test="${fw!=null}">
								<form action="fw_update.action" method="post" name="mf"
							onsubmit="return validateTitle()">
						...
						<table>
							...
							<tr>
								...
								<td>
									<input type="text" name="fw.title" value="${fw.title}">
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<select name="qxid" onChange="selectjd(${fw.jdid})" id="qx">
										<option value='0'>
											不限--
										</option>
										<option value='1' <c:if test="${fw.qxid==1}">selected="selected"</c:if> >
											海淀
										</option>
										<option value='2' <c:if test="${fw.qxid==2}">selected="selected"</c:if> >
											朝阳
										</option>
										<option value='3' <c:if test="${fw.qxid==3}">selected="selected"</c:if> >
											崇文
										</option>
										<option value='4' <c:if test="${fw.qxid==4}">selected="selected"</c:if> >
											宣武
										</option>
										<option value='5' <c:if test="${fw.qxid==5}">selected="selected"</c:if> >
											东城
										</option>
										<option value='6' <c:if test="${fw.qxid==6}">selected="selected"</c:if> >
											西城
										</option>
										<option value='7' <c:if test="${fw.qxid==7}">selected="selected"</c:if> >
											通州
										</option>
										<option value='8' <c:if test="${fw.qxid==8}">selected="selected"</c:if> >
											丰台
										</option>
										<option value='9' <c:if test="${fw.qxid==9}">selected="selected"</c:if> >
											昌平
										</option>
										<option value='10' <c:if test="${fw.qxid==10}">selected="selected"</c:if> >
											大兴
										</option>
										<option value='11' <c:if test="${fw.qxid==11}">selected="selected"</c:if> >
											门头沟
										</option>
										<option value='12' <c:if test="${fw.qxid==12}">selected="selected"</c:if> >
											石景山
										</option>
										<option value='13' <c:if test="${fw.qxid==13}">selected="selected"</c:if> >
											房山
										</option>
										<option value='14' <c:if test="${fw.qxid==14}">selected="selected"</c:if> >
											顺义
										</option>
										<option value='15' <c:if test="${fw.qxid==15}">selected="selected"</c:if> >
											密云
										</option>
										<option value='16' <c:if test="${fw.qxid==16}">selected="selected"</c:if> >
											怀柔
										</option>
										<option value='17' <c:if test="${fw.qxid==17}">selected="selected"</c:if> >
											延庆
										</option>
										<option value='18' <c:if test="${fw.qxid==18}">selected="selected"</c:if> >
											平谷
										</option>
							...
							<tr>
								...
								<td>
									<input type="text" name="fw.zj"  value="${fw.zj}"/>
									元/月
								</td>
							</tr>
							<tr>
								...
								<td>
									<select name="fw.shi" >
										<option value="1" <c:if test="${fw.shi==1}">selected="selected"</c:if> >
											1
										</option>
										<option value="2" <c:if test="${fw.shi==2}">selected="selected"</c:if>>
											2
										</option>
										<option value="3" <c:if test="${fw.shi==3}">selected="selected"</c:if>>
											3
										</option>
										<option value="4" <c:if test="${fw.shi==4}">selected="selected"</c:if>>
											4
										</option>
										<option value="5" <c:if test="${fw.shi==5}">selected="selected"</c:if>>
											5
										</option>
										<option value="6" <c:if test="${fw.shi==6}">selected="selected"</c:if>>
											6
										</option>
										<option value="7" <c:if test="${fw.shi==7}">selected="selected"</c:if>>
											7
										</option>
										<option value="8" <c:if test="${fw.shi==8}">selected="selected"</c:if>>
											8
										</option>
										<option value="9" <c:if test="${fw.shi==9}">selected="selected"</c:if>>
											9
										</option>
									</select>
									室
									<select name="fw.ting">
										<option value="1" <c:if test="${fw.ting==1}">selected="selected"</c:if>>
											1
										</option>
										<option value="2" <c:if test="${fw.ting==2}">selected="selected"</c:if>>
											2
										</option>
										<option value="3" <c:if test="${fw.ting==3}">selected="selected"</c:if>>
											3
										</option>
										<option value="4" <c:if test="${fw.ting==4}">selected="selected"</c:if>>
											4
										</option>
										<option value="5" <c:if test="${fw.ting==5}">selected="selected"</c:if>>
											5
										</option>
									</select>
									厅
							...
							<tr>
								...
								<td>
									<select name="fw.lxid">
										...
										<option value='1' <c:if test="${fw.lxid==1}">selected="selected"</c:if>>
											精装
										</option>
										<option value='2' <c:if test="${fw.lxid==1}">selected="selected"</c:if>>
											简装
										</option>
										<option value='3' <c:if test="${fw.lxid==1}">selected="selected"</c:if>>
											毛胚
										</option>
										<option value='4' <c:if test="${fw.lxid==1}">selected="selected"</c:if>>
											无家具
										</option>
									</select>
								</td>
							...
							<tr>
								...
								<td>
									<input type="text" name="fw.telephone" value="${fw.telephone}">
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<input type="text" name="fw.lxr" value="${fw.telephone}">
								</td>
							</tr>
							<tr>
								<td>
								...
								<td>
									<textarea name="fw.fwxx" rows="10" cols="40" >${fw.fwxx}</textarea>
								</td>
							</tr>
							<tr>
								<td colspan="2">
