...
-->
	...
	<form action="user_indexlogin.action" method="post" name="myForm" onsubmit="return login()">
  	<p>&nbsp;</p>
	  <table align="center">
		<tr>
			<td colspan="2">用户名：</td>
		...
		<tr>
		  <td><a href="${basePath}/page/reg.jsp">注册用户</a></td>
		</tr>
	  </table>
   </form>
</td>
				...
				<tr>
					<td colspan='3'>
						<c:if test="${index!=1}">
								...
								<a href="fw_indexquery.action?index=1">首页</a>
								<a href="fw_indexquery.action?index=${index-1}">上一页</a>
							</c:if>
							<c:if test="${index==1}">
							...
							<c:if test="${index!=max}">
								<a href="fw_indexquery.action?index=${index+1}" >下一页</a>
								<a href="fw_indexquery.action?index=${max}">尾页</a>
							</c:if>
							<c:if test="${index==max}">
